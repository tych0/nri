...
func (g *Generator) AdjustSeccompPolicy(policy *nri.LinuxSeccomp) error {
	...
	g.Config.Linux.Seccomp.DefaultAction = rspec.LinuxSeccompAction(policy.DefaultAction)
	for _, arch := range policy.Architectures {
		g.Config.Linux.Seccomp.Architectures = append(g.Config.Linux.Seccomp.Architectures, rspec.Arch(arch))
	}
	g.Config.Linux.Seccomp.ListenerPath = policy.ListenerPath
	g.Config.Linux.Seccomp.ListenerMetadata = policy.ListenerMetadata

	flags := []rspec.LinuxSeccompFlag{}
	for _, f := range policy.Flags {
		flags = append(flags, rspec.LinuxSeccompFlag(f))
	}
	g.Config.Linux.Seccomp.Flags = flags

	g.Config.Linux.Seccomp.Syscalls = nri.ToOCILinuxSyscalls(policy.Syscalls)
